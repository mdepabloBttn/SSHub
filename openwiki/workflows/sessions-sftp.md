...
- **`worker.rs`** — the blocking transport lives on a dedicated thread (`spawn_sftp_worker`) serving `SftpCommand::{ListDir, EditDownload, EditUpload, RunQueue, Remove, Mkdir, Rename, Chmod, Cancel}` and emitting `SftpEvent::{Connected, ConnectFailed, DirListing, EditDownloaded, EditUploaded, EditError, Progress, TransferDone, QueueDone, OpDone, Error}`. Progress events are throttled at 64 KiB; recursive transfers pre-plan the tree (still polling Cancel); symlinks are never descended. Remote edit uses `file_stamp` (size + mtime + regular-file bit as the server sent them) before download and again before upload; a mismatch refuses the overwrite. That check is not atomic — another writer can still win the temp-file + overwrite-rename window after the stamp is observed. `SftpTransport::file_stamp` is required: a backend must not invent `mtime: None` / `is_regular: true` from follow-`stat`, which only returns `(is_dir, size)`.
- **`model.rs`** — pure, I/O-free dual-pane state (`Pane`, `FileEntry`, `QueuedTransfer`, `Direction`) — easy to unit test.

Limitations: **ProxyJump hosts are refused** (libssh2 transport can't chain; `src/app/sftp.rs`). Remote-edit overwrite protection is check-then-act, not a compare-and-swap. SFTP is also exposed headlessly — `sshub sftp ls|get|put|rm|mkdir|rename|chmod` (see [CLI](cli.md)).
